...
data::InstallOutcome SotaUptaneClient::OstreeInstall(const Uptane::Target &target) {
  ...
  OstreePackage package(target.filename(), target.sha256Hash(), "", config.uptane.ostree_server);
...
Json::Value SotaUptaneClient::OstreeInstallAndManifest(const Uptane::Target &target) {
  ...
  operation_result["operation_result"] = result.toJson();
  Json::Value unsigned_ecu_version = OstreePackage(target.filename(), target.sha256Hash(), "", "")
                                         .toEcuVersion(target.ecu_identifier(), operation_result);
